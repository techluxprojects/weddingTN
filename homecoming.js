const HOME_LOCATION =
  "https://www.google.com/maps/dir/?api=1&destination=Amaya+Grand%2C+No.+11%2F9%2C+Malvilawatte%2C+Giriulla%2C+Sri+Lanka";
...
function readWeddingDatabase() {
  try {
    ...
    // Ignore invalid storage and fallback to default.
  }
...
function playBell() {
  try {
    ...
    // Audio is optional; the invitation still opens normally.
  }
}
...
async function loadGuests() {
  try {
    ...
    if (guestId) guestSelect.value = guestId;
  } catch (error) {
...
rsvpForm.addEventListener("submit", async (event) => {
  ...
  if (RSVP_API_URL) {
    try {
      ...
      if (!result.ok) throw new Error("RSVP failed.");
    } catch (_) {
      rsvpStatus.className = "rsvp-status";
      rsvpStatus.textContent =
    ...
    if (index >= 0) invites[index] = { ...invites[index], ...nextEntry };
    else invites.push(nextEntry);
